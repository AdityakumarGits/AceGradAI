...
const STATUS_CONFIG = {
  idle: { text: "Start recording to see live AI feedback here.", icon: Sparkles },
  listening: { text: "Listening...", icon: Radio },
  evaluating: { text: "Evaluating...", icon: Loader2 },
};

export default function EvaluationPanel({ status = "idle", feedbackItems = [] }) {
  const config = STATUS_CONFIG[status] || STATUS_CONFIG.idle;
  const Icon = config.icon;
  const isSpinning = status === "evaluating";

  return (
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0d1538]/80 backdrop-blur-xl p-5">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#d90000]/10">
            <Sparkles size={16} className="text-[#d90000]" />
          </div>
          <div>
      ...
      <div className="mt-4 flex-1 overflow-y-auto rounded-xl border border-white/10 bg-[#030712]/50 p-5">
        {feedbackItems.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <Icon
              size={26}
              className={`mb-3 text-[rgba(234,236,240,0.3)] ${isSpinning ? "animate-spin" : ""}`}
            />
            <p className="text-sm text-[#eaecf0]/60">{config.text}</p>
          </div>
        ) : (
          <div className="space-y-3">
            {feedbackItems.map((item, i) => (
              <div key={i} className="rounded-lg border border-white/10 bg-white/5 p-4">
                <p className="text-sm leading-6 text-[#eaecf0]/80">{item}</p>
              </div>
            ))}
          </div>
        )}
